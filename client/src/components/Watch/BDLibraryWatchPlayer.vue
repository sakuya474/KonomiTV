<template>
    <div class="watch-player" :class="{
        ...
        <div class="watch-player__button"
            ...
            <!-- 字幕ON/OFFボタン -->
            <div v-ripple class="switch-button switch-button-subtitle"
                :class="{ 'active': playerStore.is_subtitle_on }"
                :disabled="playerStore.subtitle_tracks.length === 0"
                v-ftooltip.bottom="'字幕ON/OFF'"
                @click="playerStore.toggleSubtitle()">
                <Icon class="switch-button-icon" icon="mdi:closed-caption" width="28px" />
            </div>
        </div>
    </div>
</template>
