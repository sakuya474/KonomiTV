...
export type PlayerEvents = {
    ...
    // BD視聴時: 指定位置にシークすることを要求する
    SeekTo: {
        seconds: number;  // シーク位置 (秒)
    }
};
...
const usePlayerStore = defineStore('player', {
    ...
    actions: {
         ...
         */
        toggleSubtitle(): void {
            if (this.subtitle_tracks.length === 0) return;
            this.is_subtitle_on = !this.is_subtitle_on;
            // 実際のプレイヤーコントローラへの反映は別途実装
        },
    }
});
