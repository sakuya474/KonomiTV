...
class MediaSessionManager implements PlayerManager {
    ...
    public async init(): Promise<void> {
        ...
        // ただし BD 視聴など recorded_program.id が未設定 (-1) の場合はアイコンにフォールバック
        const useAppIcon = (this.playback_mode === 'Live') || (player_store.recorded_program?.id ?? -1) < 0;
        const artwork = useAppIcon ? [
