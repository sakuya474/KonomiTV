...
## 本 Fork の特徴
本フォークでは、以下の機能を実装しています。自宅で運用するうえで欲しい機能を思ったまま作ったフォークなのであしからず。
- **TSReplace エンコード機能**: H.262-TS 形式の録画ファイルを H.264/HEVC にエンコードする機能
- **キャプション画像ページ**: キャプチャ画像の管理
- **番組表**: 番組表の表示
- **Discord連携**: Discord での通知や録画予約の管理などが可能
  
### TSReplace エンコード機能

**rigaya氏のTSReplaceを利用し、H.262-TS 形式の録画ファイルを H.264/HEVC に自動・手動エンコードする機能を搭載**

- **自動エンコード**: 新しい録画番組を自動的に検出して、バックグラウンドでエンコードを実行
- **ファイルサイズ大幅削減**: 通常 50-80% のサイズ削減を実現。映像のみをエンコードするため、音声や字幕はそのまま保持
- **ハードウェアエンコーダー**: AMD/NVIDIA/Intel GPUに対応し、高速エンコードが可能

#### 使用方法

1. **設定画面** から「TSReplace エンコード」を選択
2. **自動エンコード** を有効にして、コーデック・エンコーダーを選択
3. **エンコードオプション** を必要に応じて調整
4. 手動でエンコードする場合は、録画番組の詳細画面から「録画ファイルを再エンコード」を実行

> [!Warning]
> **再エンコード機能を使用するには、事前に [TSReplace](https://github.com/rigaya/tsreplace) を導入する必要があります。  
[リリースページ](https://github.com/rigaya/tsreplace/releases) から最新版をダウンロードして、```KonomiTV\server\thirdparty\tsreplace``` となるように配置してください。**

### 通知機能
- **起動/終了通知**: KonomiTV サーバーの起動・終了時に、指定した Discord チャンネルへ通知を送信
- **録画開始/終了通知**: 予約番組の録画開始・終了時に、指定した Discord チャンネルへ通知を送信

> **注意**: これらの機能を利用するには`/setting channel`でチャンネルの設定をする必要があります。

### Discord スラッシュコマンド

`/help` でコマンド一覧を表示できます。利用可能なコマンドは以下の通りです：
...
#### ⚙️ 設定コマンド
- `/setting channel` - 通知チャンネルを設定
- `/setting notify` - 予約通知の有効/無効を切り替え

#### 📺 ビューコマンド
- `/view channel_list` - 指定タイプのチャンネル一覧を表示（地デジ(GR), BS, CS）
- `/view channel_now` - 指定されたチャンネルの現在と次の番組情報を表示
- `/view recorded_info` - 録画済み番組一覧を表示
- `/view search_programs` - 番組検索を実行
- `/view reservation_list` - 録画予約一覧を表示

#### 🛠️ メンテナンスコマンド(個別にユーザーの許可をする必要があります。)
- `/maintenance restart` - サーバーを再起動
- `/maintenance shutdown` - サーバーを終了
- `/maintenance epg_acquire` - EPG取得を開始
- `/maintenance epg_reload` - EPG を再読み込み
---------------------
...
## 目次 <!-- omit in toc -->
- [本 Fork の特徴](#本-fork-の特徴)
  - [TSReplace エンコード機能](#tsreplace-エンコード機能)
    - [使用方法](#使用方法)
  - [通知機能](#通知機能)
  - [Discord スラッシュコマンド](#discord-スラッシュコマンド)
    - [🔧 ユーティリティコマンド](#-ユーティリティコマンド)
    - [⚙️ 設定コマンド](#️-設定コマンド)
    - [📺 ビューコマンド](#-ビューコマンド)
    - [🛠️ メンテナンスコマンド(個別にユーザーの許可をする必要があります。)](#️-メンテナンスコマンド個別にユーザーの許可をする必要があります)
- [設計思想](#設計思想)
- [動作環境](#動作環境)
  - [サーバー](#サーバー)
  - [クライアント](#クライアント)
...
```bash
curl -LO https://github.com/tsukumijima/KonomiTV/releases/download/v0.2.3/KonomiTV-Installer.elf
