...
# ルーターの追加
app.include_router(BDLibraryRouter.router)
app.include_router(BDLibraryStreamsRouter.router)
...
async def check_and_notify_reservations():
    ...
    try:
        ...
        if config.general.recorder == 'EDCB':
            # ReservationsAPI は依存として CtrlCmdUtil を内部解決するため引数不要
