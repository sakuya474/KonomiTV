...
# KonomiTV-master 直下の server/misc/ 配下に配置
from misc.BDLibraryScanner import load_bd_folders_from_config, scan_async
...
@router.post(
    '/run-bd-library-scan',
    summary = 'BDライブラリフォルダ一括スキャン API',
    status_code = status.HTTP_204_NO_CONTENT,
)
async def BDLibraryScanAPI():
    """
    BD ライブラリの保存先フォルダを設定から読み込み、一括スキャンを実行して
    メタデータをDBへ反映する。KonomiTVの元仕様と同一の動作。
    このメンテナンス機能は管理者ユーザーでなくてもアクセスできる。
    """

    # 設定からBDフォルダ一覧を読み込み
    folders = load_bd_folders_from_config()
    # 非同期スキャンを実行（内部でDB更新を行う）
    await scan_async(folders)
